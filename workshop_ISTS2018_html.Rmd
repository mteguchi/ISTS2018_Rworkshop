---
title: "ISTS 2018 Introductory R Workshop"
author: "Tomo Eguchi"
date: '`r Sys.Date()`'
...
#Welcome to the world of R computing
Just a little bit about my programming experience... I learned how to program 20 some years ago using Matlab. I also learned C, Fortran, BUGS, and JAGS and used them along with Matlab. After talking with my friends and colleagues, however, I moved from Matlab to R and have not had regrets. I still use JAGS through R for Bayesian analyses. I recommend you picking R and start using it every day. I think R is a vey useful tool in every ecologist's tool box.

I have tried to preach some people to use R in the last few years with some success. There are a lot of useful information out there with respect to learning R. RStudio combines R and tools that work with R in one package. It is very user-friendly  software. I hope it does not become too large as software always does. We will touch on some functions within it but we will not touch on many others in our workshop. Please visit their website - there are some excellent Webinar recordings (https://www.rstudio.com/resources/webinars/). There are also some excellent books, e.g., the Art of R Programming by Norman Matloff. A free version is available at http://diytranscriptomics.com/Reading/files/The%20Art%20of%20R%20Programming.pdf. 

In this workshop, we will get comfortable using R, learn how to manipulate data, conduct statistical analyses (linear models), and to make publication-ready plots. 

#Setting up RStudio
Before we get started on learning how to use R and RStudio, let's set thigns up. 

In RStudio, you have four panes, like so:

```{r RStudio_general, echo=F, out.width='400px'}
knitr::include_graphics('images/Rstudio_general.png')
```

You can arrange what goes into each pane/panel using Tools-Global Options...-Pane Layout:

```{r RstudioGlobalOptions, echo=F, out.width = '400px'}
knitr::include_graphics('images/RStudio_GlobalOptions.png')
```

You can arrange all the windows in wherever you like. Background and text colors can be changed within the same global options menu. I like dark background and white letters, which seem to be easy on my eyes. 

#A teaser on R Markdown and Knitr
This is a document made with R Markdown and Knitr using RStudio, demonstrating how PDF, html, or MS Word documents can be created using R. This will eliminate copying and pasting results and plots when writing a document. When a new dataset is available, or find an error in your data, all you need to do is to fix your data and run the document (knit) in R. All corrections will be made simulataneously, including analyses, statistics in text, and figures. After making the first edition of this document, I decided to teach this workshop using RMarkdown. I think this approach is the best way to conduct data analyses because it makes easier to conduct reproducible science. We will use the "Notebook" feature of RStudio during the workshop. 
...
Of course, the previous chunk did not do anything because we did not put any code in there, except comment lines. A hash (#) is used to indicate a comment line in R. When you click on "Knit" button at upper left in RStudio, you will see that a Word or PDF document is created (I have not tested this on Mac computers). 
...
Let's start talking about data analyses and coding. But first thing first... we need to have our data organized in a way that we can access them easily. We also need to have our stuff organized such that analyses can be repeated easily (reproducibility of analyses). 
...
Regardless of your choice of operating system, i.e., Windows, Mac, Unix, Linux, ..., the basic structure is the same. In recent years (Windows was the last one, maybe?), all OSs use user specific home directory structure. For example, my home directory in a Windows computer looks like this:
...
Note you need to use quotation marks (single or double) when installing a package but no quotation marks are necessary (but can be used) when calling them into your working environment.

It would be best to load the only packages that you need in the current session to keep R efficient - all the files in packages are stored in the computer's memory. If you need just one or two functions from a package, you can use '::' to specify a package and function without loading the entire package. You have to have the package available locally. In other words, you have to do "install.packages()" before the package can be available for use. For example, assume you have installed the ggmap package. Then you can do something like this:

```{r double_colons, echo=TRUE}
# This downloads and creates a map of San Diego Bay, California, USA. You need to installl
# the ggmap package, if you haven't done so. (install.packages("ggmap"))
if (!file.exists('maps/sdbay.rds')){
  ...
  sdbay.all <- readRDS('maps/sdbay.rds')
}

ggmap::ggmap(sdbay.all)
```

Note that the map was downloaded from the Internet. You can combine these maps with GIS shape files also - we may touch on this topic later. The downloaded maps can be stored on your hard drive (I think the stored maps will become unavailable after a certain time period.)  It may be a good practice to create a dedicated directory for these downloaded files.
...
Even though "3030" seems like a number, it is stored as a string of characters in the data frame (confirm this by str(growth_data_clean)). Consequently, I used the quotation marks around 3030. It turned out the turtle was caught 8 times over the years. 
...
This effectively linearly interpolates data without adding a "point" in the plot. If you insist... I will also introduce another tool "tidyverse", which is a combination of packages (ggplot21, dplyr, tidyr, readr, purrr, tibble, stringr, forcats) to deal with data manipulations. This package also allows you to use pipes (from magrittr package as part of dplyr) to send output from one line to the next. It's easier to understand how this works through an example:  
...
known_SCLs <- filter(turtle3030, Date == as.Date("2002-03-20") |
...
When you look at n_caught, you notice that this object is a tibble. What is a tibble? A tibble is a kind of data frame. There are some nice features that are absent in regular data frames.  There are some frustrating features also... You can easily force a tibble into a data frame by using the function data.frame():

```{r data.frame, echo=TRUE}
growth_data_clean %>% filter(., SCL>60) %>% 
  count(., Turtle_ID) %>% data.frame(.) -> n_caught3

# again, the periods in each function call can be removed:
growth_data_clean %>% filter(SCL>60) %>% 
  count(Turtle_ID) %>% data.frame() -> n_caught4
...
Many rows, namely ```r dim(growth_data_clean)[1] - dim(datCmNoNA)[1]``` rows were removed by having at least one NA. A quiz; how did I figure out the number, i.e., ```r dim(growth_data_clean)[1] - dim(datCmNoNA)[1]```? 
...
growth_data_clean %>% group_by(Turtle_ID) %>% 
  summarize(meanSCL = mean(SCL), meanMass = mean(Weight), 
            firstSCL = first(SCL), firstMass = first(Weight),
            lastSCL = last(SCL), lastMass = last(Weight),
            firstDate = first(Date), lastDate = last(Date)) -> stats_growth_data
...
We can figure out the IDs of these turtles by looking at the out.lier data frame (or tibble to be exact). These are "1304", "13581", "LB-382", and "X-129_X-130". The original data frame (growth_data_clean) also contained "Turtle_ID" field. So, we want to remove lines with these IDs. There are some useful functions in dplyr; inner_join(), left_join(), right_join(), semi_join(), anti_join(), and full_join(). We want to have all rows in growth_data_clean where Turtle_ID does not match with those in Turtle_ID of out.liers. (Complicated!)

```{r outlierRemoval_4, echo=TRUE}
# we use anti_join for this kind of extraction:
growth_data_clean_1 <- anti_join(growth_data_clean, out.liers, 
                                 by = "Turtle_ID")
...
For this example, I will use stranding records of loggerhead turtles along the west coast of the U.S. These data were collected as part of the stranding network in the area. They are stored in a dedicated database at the Southwest Fishries Science Center. I extracted data from the database (note, SQL databases can be accessed from R using open database connection, a.k.a., ODBC, which eliminates the use of intermediate steps, such as MS Access).  Because loggerheads are mostly found in warmer waters, strandings also occur in the southern part of the region. 
...
Note that there was a blank entry in the state variable. Let's remove that one and create a new factor variable for year, which will be used to change colors in a plot later:

```{r strandingData2, echo=T}
dat0 %>% filter(., State != '') %>%   # new data frame without a blank state
  mutate(Year = as.factor(Year_Initially_Observed)) -> dat0.state # a factor var.
```
...
dat0.state %>% 
  filter(., Alive_Released == 'FALSE' &
                 !is.na(Latitude)) %>%
  select(., State, Year, Species_Code,
         Latitude, Longitude,
         Weight,
         Curved_Carapace_Length,
         Straight_Carapace_Length) -> dat1

summary(dat1)
```

Once the data look good, we can start plotting a base map, then overlay the stranding records on top of it. We fetcth a map from the Internet:

Using get_map() function from ggmap, you provide the center longitude and latitude and how big the map should be by providing a value in the zoom input. This step is fine tuned through a trial and error. Please look at help file for get_map for more information on different settings. 

```{r getMaps, echo=TRUE, cache=TRUE}
...
The get_map function downloads necessary data from the Internet, which will be used in the following lines (ggmap) to visualize:

```{r plotMaps, echo=TRUE}
...
Once maps are downloaded, overlaying your data on top of them is an easy task with ggplot. Just as before, we overlay data points on the map using ggplot:
